      ...
      <div class="images-section" :class="{ 'fullscreen-active': listFullscreen }">
        <div class="section-header">
          ...
          <div class="header-right">
            ...
            <button class="fullscreen-btn" @click="listFullscreen = !listFullscreen" :title="listFullscreen ? '退出全屏' : '全屏'">
        ...
        <!-- List view -->
        <div v-else-if="filteredImages.length > 0 && viewMode === 'list'" class="image-list">
          <!-- List header -->
          <div class="list-header" :style="listGridStyle">
            ...
            <div class="col col-caption">
              描述
              <button class="col-filter-btn" @click="openColFilter('caption')" :class="{ active: colFilters.caption }" title="筛选">
                <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.5"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
              </button>
            </div>
          ...
          >
            ...
            <div class="col col-caption" :title="(img.extra || {}).caption">{{ (img.extra || {}).caption || '—' }}</div>
      ...
      <div v-if="detailImage" class="detail-content">
        <div class="detail-preview">
          ...
          <div class="preview-main">
            <img
              :src="datasetImageUrl(dataset.id, detailImage.relative_path, true)"
          ...
          >
            <img
              ...
              :src="datasetImageUrl(dataset.id, detailImage.mask_relative_path, true)"
        ...
        <div class="detail-fields">
          ...
          <div v-for="ax in axes" :key="ax.axis_key" class="field readonly">
            <label>{{ ax.axis_name }}</label>
            <span>{{ (detailImage.labels || {})[ax.axis_key] || '—' }}</span>
          </div>
          <div class="field readonly">
            <label>尺寸</label>
          ...
          <div v-if="detailImage.extra && detailImage.extra.caption" class="field readonly">
            <label>描述</label>
            <textarea class="caption-text" readonly :value="detailImage.extra.caption" rows="4"></textarea>
          </div>
        </div>
      </div>
    </el-dialog>
...
// Lazy image loading: only load images that enter the viewport (within 50px margin)
...
watch(currentPage, () => {
  // After DOM update for new page, observe new images
  setTimeout(() => observeAllImages(), 0)
})

const colFilterLabel = computed(() => {
  const labels = { keyword: '关键词', source_page_title: '来源页', caption: '描述' }
...
const listGridStyle = computed(() => {
  const cols = ['60px', '60px', '120px', ...axes.value.map(() => '1fr'), '1fr', '1fr', '100px']
...
const filteredImages = computed(() => {
  ...
  return allImages.value.filter(img => {
    for (const [key, val] of Object.entries(filters)) {
      ...
      else if (key === 'caption') cellVal = (img.extra || {}).caption || ''
...
.col.col-caption { color: var(--text-tertiary); font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
